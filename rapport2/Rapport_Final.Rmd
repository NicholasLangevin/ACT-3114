...
Pour l'interprétation de ce modèle, étant plus demandant que pour le modèle de base étant donné le nombre élevé d'arbre entrainé, nous utiliserons l'importance des variables, les graphiques de dépendance partielle et la statistique H de Friedman.

<<<<<<< HEAD
L'importance des variables nous permet de comprendre l'influence des variables explicatives sur la prévision. Ainsi, on peut visualiser les variables du modèles ayant le plus grand impact sur la prévision. De la \autoref{tbl_iml_gbm.tex} et de la \autoref{imp_gbm.png}, on voit que les variables les plus significatives sur ce modèles sont définitivement *prem_index* et *vehicl_region*, tout comme *prem_freqperyear*, *polholder_age* et *vehicl_age* et d'autres mais qui ont une mesure d'importance moins considérable que les deux premières. Les valeurs sur laquelles ont ce fient pour tirer cette conclusion vient du pourcentage de variance relative des variables prises individuellement sur la prévision. Plus le pourcentage de variance relative est élevée, plus la valeur de la variable pourra avoir un impact sur la prévision. Dans le but de mieux comprendre l'effet individuel des variables ayant le plus grand impact, on utilisera l'outil des graphiques de dépendances partielles en se concentrant principalement sur les variables *prem_index* et *vehicl_region*. 
=======
L'importance des variables nous permet de comprendre l'influence des variables explicatives sur la prévision. Ainsi, on peut visualiser les variables du modèles ayant le plus grand impact sur la prévision. De la \autoref{fig:imp-gbm}, on voit que les variables les plus significatives sur ce modèles sont définitivement *prem_index* et *vehicl_region*, tout comme *prem_freqperyear*, *polholder_age* et *vehicl_age* et d'autres mais qui ont une mesure d'importance moins considérable que les deux premières. Les valeurs sur laquelles ont ce fient pour tirer cette conclusion vient du pourcentage de variance relative des variables prises individuellement sur la prévision. Plus le pourcentage de variance relative est élevée, plus la valeur de la variable pourra avoir un impact sur la prévision. Dans le but de mieux comprendre l'effet individuel des variables ayant le plus grand impact, on utilisera l'outil des graphiques de dépendances partielles en se concentrant sur les variables *prem_index* et *vehicl_region*. 
>>>>>>> a38bc5183f80fce1a04ffca49a2694c27d19494d
...
```{r pdp_policy_age, results='asis', fig.height=3, fig.cap="Graphique de dépendance partielle pour la variable policy.age"}
...
On peut voir dans la \autoref{int_prem_index} que seule les interractions avec la variable *prem_index* est illustrée. En effet, le grahique obtenu avec la variable *vehicl_region* a aussi été obtenu pour analyser les interractions ayant la force la plus élevée avec cette variable seulement, aucune d'elles ne permettaient une interprétation intéressante. L'interprétation des interractions importantes se fera donc sur les interractions sur *prem_index* ayant la plus grandes forces d'interraction, soit *vehicl_region:prem_index*, *prem_freqperyear:prem_index*.  À l'aide de l'outil des graphiques de dépendances partielles on obtient les figures suivantes. 

```{r int_prem_index, results='asis', fig.height=4, fig.cap="\\label{}Graphique de l'importance des interractions entre la variables prem.index et les autres variables sur la prévision"}
plot(int.prem_index)
```

<<<<<<< HEAD
De la \autoref{pdp_interraction}, on peut observer que certaines régions possède un creux ou la probabilité de résignation est beaucoup plus faible lorsque le pourcentage d'augmentation de la prime ce situe entre 0 et 50% dont les régions 1, 2, 8, 9, 12, 13 et 14. Pour ce qui est des autres régions, la probabilité de résignation demeure assez stable et faible lorsque le pourcentage d'augmentation est supérieur à 0%. Un dernier constat intéressant concerne d'ailleurs la région 6 et 7, ou la probabilité de résignation est plus faible si la prime augmente que si elle diminue ce qui est contre-intuitif.

Finalement, à partir de la \autoref{pdp_interraction2}, on peut observer que plus les assurés effectuent le paiement de leur prime en plusieurs paiement, plus a probabilité de résignation est stable selon le pourcentage d'augmentation de leur prime.
...
<<<<<<< HEAD
De cette interprétation, on comprend que les variables ayant le plus grand impact sur la prévision dans ce modèles sont les variables *prem_index*, *vehicl_region* et l'interraction entre ces 2 variables.
=======
>>>>>>> a38bc5183f80fce1a04ffca49a2694c27d19494d
\newpage
