...
ACT-3114/rapport2/Rapport_Final.Rmd
@NicholasLangevin NicholasLangevin Debut de la presentation
b813b90 2 days ago
@NicholasLangevin@CharlesCo02@AndreanneLarouche
579 lines (488 sloc)  26.7 KB
...
\begin{align}
    \min\limits_{(\beta_0, \beta_j)} -\left[ \sum_{i=1}^n 
    y_i (\beta_0 + \sum_{j=1}^p \beta_j x_j) - \ln(1 + e^{(\beta_0 +
    \sum_{j=1}^p \beta_j x_j)}\right] + \lambda \sum_{j=1}^p |\beta_j|.
\end{align}
...
L'optimisation des deux hyperparamètres restants est ensuite fait à partir de la méthode **LGOCV**, soit _left one group out cross validation_. **C'est donc une méthode de validation croisée qui permet de tester des combinaisons d'hyperparamètres choisies.(À VALIDER)** Le choix des hyperparamètres optimaux se fera à l'aide de la métrique _AUC_ qui représente l'aire sous la courbe ROC. Le choix des paramètres est effectué selon l'AUC obtenue la plus élevée. Ainsi, une grille de toutes les combinaisons possibles d'hyperparamètres à tester a été crée et la fonction **train** de R nous a donnée la meilleure combinaison parmis celles proposées.

Ainsi, le modèle choisit utilisera un $n.trees(T) = 1600$, un $interaction.depth(d) =2$, un $shrinkage(\lambda) = 0.01$, un $n.minobsinnode = 20$ et utilisera la déviance négative d'une loi bernoulli comme fonction de perte.



**Voir pour ajouter le graphique de l'Accuracy en fonction des combinaisons d'hyperparamètres testés plot(gbmfit1)**
