...
lasso.names0 <- data.frame(Coefficients = coeff_names, Positif = coeff_positif,
                           Negatif = coeff_negatif, "Niveau inclu dans Intercept" = niv_inclu)
...
Interprétation de la probabilité de résignation à partir du lien (logistique) du glm:

$\eta = ln\left(\frac{\pi_{res}}{1 - \pi_{res}}\right) \quad \Leftrightarrow \quad \pi_{res} = \frac{e^{\eta}}{1 + e^{\eta}}$ (fn expit)

La fonction expit est croissante lorsque $\eta$ augmente (prob augmente), décroissante lorsque que $\eta$ est décroissant.

$\eta = \sum\limits_{j=1}^p \beta_{j} x_{j}$

Coefficients negatif -> fait baisser la prob, Coefficients positif -> augmente la prob
...
L'importance des variables nous permet de comprendre l'influence des variables explicatives sur la prévision. Ainsi, on peut visualiser les variables du modèles étant plus significatives dans l'élaboration du modèle. 

Les graphiques de dépendance partielle ( _DPD_) sont un outil plus spécifique. Étant donné que l'importance des variables ne nous permet pas de comprendre l'effet plus spécifique d'une variable explicative sur la prévision, c'est à l'aide de cette outil qu'on peut interprété cette partie du modèle. Cet outil est utilisé pour mieux comprendre l'effet global d'une variable explicative sur la prévision et peut être interprété comme la moyenne des ICEs. Le désavantage de cet outil est qu'il y a beaucoup de courbe surperposées. Ainsi, il est difficile de voir l'effet individuel de la prévision sur les observations. Si nécessaire on peut aussi faire les graphiques d'espérance conditionnelle pour pouvoir interpréter l'effet individuel d'une variable par prévisions et non pour les prévisions en générales. C'est en observant les graphique de dépendance partielle qu'on pourra savoir si les graphiques d'espérance conditionnelle individuelle pourrait être utile pour l'interprétation du modèle.

Nous aurons également recourt à la statistique H de Friedman pour pouvoir estimer la force d'une interraction en mesurant la quantité de la variance qui provient de cet interraction dans la prévision. Ce qui nous permettera plus spécifiquement d'observer les variables qui interragissent avec les variables les plus importantes pour expliquer la prévision, soit **prim_last** et **vehicl_region**. Sachant que la profondeur maximale de l'arbre pour ce modèle est de `r gbmfit_Bern$bestTune["interaction.depth"]`, il pourrait y avoir des interraction sur jusqu'à `r gbmfit_Bern$bestTune["interaction.depth"]` variables différentes. On pourra ensuite réutiliser les DPD sur les interractions qui nous intéressent.
...
Ajout des graphiques pdp nécessaire pour les interractions ayant la plus grande importance (1-2 graphiques max)
