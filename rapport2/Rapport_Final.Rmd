...
Observations & Renouvellemnt & Résignation \\
...
Il est intéressant de voir que les variables **prem\_pure** et **prem\_last**
...
**prem\_index = prem\_final / prem\_last** avait été crée.  Nous verrons plus
...
- *Polholder_age* : Lorsque le sexe de l'assuré n'aura aucun impact sur la probabilité lorsque l'assuré est un femme tandis que lorsque l'assuré est un homme, la probabilité de résignation aura tendance à augmenter ;
...
- *prem_freqperyear* : pour ce qui est de cette variables, une fréquence de paiement choisie à une fois par année n'aura aucun impact sur la probabilité de résigner. Les assurés faisant des paiements semestrielles auront une probabilité moins élevée de résigner leur police et comparativement à ceux faisant leur paiement 3 ou 4 fois par année qui auront une probabilité plus élevée.
...
#coefficients.glm <- summary(glm_base)$coefficients[,1][which(abs(summary(glm_base)$coefficients[,1]) > 0.15)]
#coefficients <- as.data.frame(coefficients.glm)
#variables <- c("Intercept", "polholder.age", "polholder.BMCevolstable", "polholder.BMCevolup", #"polholder.diffdrivercommercial", "polholder.diffdriverlearner 17", "polholder_diffdriveronly partner", #"polholder.diffdriversame", "polholder.diffdriveryoung drivers", "polholder.genderMale", "polholder_jobnormal", #"policy_age", "prem_final", "prem_freqperyear.L", "prem_freqperyear.Q", "prem_freqperyear.C", "prem_last", #"prem_market", "vehicl_age", "vehicl_regionReg2", "", "", "", "", "", "" , "", "", "", "")
#save(coefficients, file = "../src/01-glm/tableau_coefficient.tex")
```
...
Pour l'interprétation de ce modèle, nous utiliserons l'importance des variables, les graphiques de dépendance partielle et la statistique H de Friedman.

L'importance des variables nous permet de comprendre l'influence des variables explicatives sur la prévision. Ainsi, on peut visualiser les variables du modèles étant plus significatives dans l'élaboration du modèle. De la (mettre les 2 REFERENCES ICI), on voit que les variables les plus significatives sur ce modèles sont *prem_index* et *vehicl_region* 
...
Les graphiques de dépendance partielle ( _DPD_) est un outil plus spécifique. Étant donné que l'importance des variables ne nous permet pas de comprendre l'effet plus spécifique d'une variable explicative sur la prévision, c'est à l'aide de cette outil qu'on peut interpréter cette partie du modèle. Cet outil est ainsi utilisé pour mieux comprendre l'effet global d'une variable explicative sur la prévision et peut être interprété comme la moyenne des ICEs, soit l'espérance conditionnelle individuelle. 

```{r pdp_prem_index, results='asis', fig.height=4, fig.cap="Graphique de dépendance partielle pour la variable prem.index"}
plot(gbm_opti_Bern, 19, gbmfit_Bern$bestTune["n.trees"], type ="response")
```

```{r pdp_vehicl_region, results='asis', fig.height=4, fig.cap="Graphique de dépendance partielle pour la variable vehicl.region"}
plot(gbm_opti_Bern, 18, gbmfit_Bern$bestTune["n.trees"], type ="response")
```

```{r pdp_policy_age, results='asis', fig.height=4, fig.cap="Graphique de dépendance partielle pour la variable policy.age"}
plot(gbm_opti_Bern, 6, gbmfit_Bern$bestTune["n.trees"], type ="response")
```


Pour ce qui est de la statistique H de Friedman, elle  est utilisé pour pouvoir estimer la force des interractions entre une variable précisement et tous les autres variables en mesurant la quantité de la variance qui provient de cet interraction dans la prévision. On peut ainsi observer les variables qui interragissent avec les variables les plus importantes pour expliquer la prévision, soit les variables *prem_index* et *vehicl_region*. Sachant que la profondeur maximale de l'arbre pour ce modèle est de `r gbmfit_Bern$bestTune["interaction.depth"]`, il pourrait y avoir des interraction sur jusqu'à `r gbmfit_Bern$bestTune["interaction.depth"]` variables différentes. On pourra ensuite réutiliser les DPD sur les interractions qui nous intéressent.

On peut voir dans les (2 FIGURES SUIVANTES : Mettre une ref) que les interractions ayant la force d'interraction la plus élevée sur la prévision sont vehicl_region:prem_index, prem_freqperyear:prem_index et prem_final:vehicl_region. En effet, on observant la force d'interraction sur les autres interractions sur les variables les plus importantes pour ce modèle, on remarque que les interractions mentionnées précédemment sont celles ayant la force la plus élevée mais aussi les seules menant à une analyse interressante sur l'impact sur la prévision par les (METTRE REFERENCE ICI DES 3 GRAPHIQUES PDP SUIVANTS) représentant les _PDP_ de ces interractions. On remarque que ..... À COMPLETÉ PAR L'ANALYSE DES 3 GRAPHIQUES SUIVANTS.

```{r int_prem_index, results='asis', fig.height=4, fig.cap="Graphique de l'importance des interractions entre la variables prem.index et les autres variables sur la prévision"}
plot(int.prem_index)
```

```{r int_vehicl_region, results='asis', fig.height=4, fig.cap="Graphique de l'importance des interractions entre la variables vehicl.region et les autres variables sur la prévision"}
plot(int.vehicl_region)
```


```{r dpd_interraction, results='asis', fig.heigth=4, fig.cap="Graphique de dépendance partielle pour l'interraction entre la variable vehicl.region et la variable prem.index"}
plot(gbm_opti_Bern, 18:19, gbmfit_Bern$bestTune["n.trees"], type="response")
```

```{r dpd_interraction2, results='asis', fig.heigth=4, fig.cap="Graphique de dépendance partielle pour l'interraction entre la variable prem.freqperyear et la variable prem.index"}
plot(gbm_opti_Bern, c(10,19), gbmfit_Bern$bestTune["n.trees"], type="response")
```

```{r dpd_interraction3, results='asis', fig.heigth=4, fig.cap="Graphique de dépendance partielle pour l'interraction entre la variable prem.final et la variable vehicl.region"}
plot(gbm_opti_Bern, c(9,18), gbmfit_Bern$bestTune["n.trees"], type="response")
```
