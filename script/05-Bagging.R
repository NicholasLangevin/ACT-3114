...
res_opt_bag <- matrix(nrow = 49, ncol = 2)
j <- 1
for(node in 2:50){
    auc_opt <- numeric(4)
    for(k in 1:4){
    ...
    res_opt_bag[j, ] <- c(node, mean(auc_opt))
    j <- j + 1
}

saveRDS(res_opt_bag, file = paste0(getwd(), "/src/05-Bagging/result_opt_bag_save.rds"))
nodesize_opt <- res_opt_bag[which.max(res_opt_bag[, 2])] # 34
...
saveRDS(mod_bag_final, file = paste0(getwd(), "/src/05-Bagging/mod_bagging.rds"))
