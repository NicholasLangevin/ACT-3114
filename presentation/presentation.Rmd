...
nocite: | 
  @CASdatasets, @glmnet.intro, @Caret, @IntroAppSupervise
output: 
...
- Importance de la prédiction de résignation
...
## Variables concernant la prime
### Écart entre la prime chargée et la prime pure
```{r}
...
- Estimateurs $\beta_{i}$ 
<font size = "4">
$$    \mathcal{L}(\beta_{0}, \ldots, \beta_{p}) = \max\limits_{(\beta_0, \beta_j)} exp\left\{ \sum_{i=1}^n \left( y_{i} (\beta_{0} + \sum_{j=1}^p \beta_{j} x_{j} ) - ln \left(1 + e^{\beta_{0} + \sum\limits_{j=1}^p \beta_{j} x_{j}} \right)\right) \right\}$$
</font>
- TRV 99\% pour la sélection de variable
...
- Optimisation de l'hyperparamètre $\lambda$ avant l'estimation des coefficients

- Estimateurs $\beta_{i}$ 

<font size = "4">
$$\max _{\left(\beta_{0}, \beta_{j}\right)} \mathcal{L}(\beta_{0}, \ldots, \beta_{p}) - \lambda \sum_{j=1}^{p}\left|\beta_{j}\right|$$
</font>
...
<center>
![](lambda_opti.jpg){width=550px}
</center>

## Modèle linéaire avec régularisation

### Variables avec coeffcient nul

<br/>
<center>
![](selection_lasso.jpg){width=600px}
...
- Processus de partitionnement récursif binaire
- Deux fonctions de perte testée
$$
\mathcal{L}_G = \sum_{k=1}^K \widehat{P}_{mk} (1 - \widehat{P}_{mk}) \qquad \mathcal{L}_D = - \sum_{k=1}^K \widehat{P}_{mk} \ln( \widehat{P}_{mk})
$$
<br/>
- Complexité de l'arbre sera optimisée en fonction de deux paramètres
    - paramètre de complexité (cp)
    - nombre minimal d'observations dans une feuille (minbucket) 
...
- Paramètre optimisé: taille minimale d'un noeud (nodesize)
...
- Taille de l'échantillon boostrap de 50\%
...
- Deux hyperparamètre à optimiser
    - taille minimal d'un noeud (nodesize)
    - nombre de prédicteurs choisi aléatoirement (mtry) 


## Boosting de gradient stochastique

- Amélioration à l'itération t
<font size = "4">
$$
   \sum_{i=1}^n \mathcal{L} \left\{ y_i, \widehat{f}_{t-1}(x_i) + \widehat{f}_{\text{arbre}}^t(x_i) \right\}
$$
</font>
- Avec fonction de perte 
<font size = "4">
$$
    \mathcal{L} = - \sum_{k=1}^K \widehat{P}_{ik} \ln( \widehat{P}_{ik})
$$
</font>

- Taux d'apprentissage <font size = "5"> $\lambda = 0.005$ </font>, sous-échantillonnage de 75\% 

- Deux hyperparamètre à optimiser
    - nombre d'arbre (n.trees)
    - profondeur maximal des arbres (d) 
...
- Modélisation avec lien logistique 
...
- Échantillon test 20\%
- Comparaison avec la courbe ROC et l'AUC

<br/>
<center>
<img src="comparaison_ROC.jpg" width="400" />  <img src="comparaison_AUC.jpg" width="500" height = 300 /> 
</center>
>>>>>>> e485c38... presentation charles

- Impact des coefficients sur la prévision

<center>

![](InterpretationGLM.PNG){width=800px}
...
![](imp_gbm.png){width=650px}


## Modèle de boosting de gradient stochastique 
### Graphique de dépendance partielle sur prem_index

![](pdp_prem_index.png){width=700px}


## Modèle de boosting de gradient stochastique
### Graphique de dépendance partielle sur vehicl_region

![](pdp_vehicl_region.png){width=700px}

## Modèle de boosting de gradient stochastique
### Graphique de dépendance partielle sur policy_age

![](pdp_policy_age.png){width=700px}


## Modèle de boosting de gradient stochastique
### Graphique de dépendance partielle sur prem_freqperyear

![](pdp_prem_freqperyear.png){width=700px}
...
## Modèle de boosting de gradient stochastique
### Interaction entre prem_index et vehicl_region (DPD bivarié)

![](interraction.png){width=700px}


# Performance des meilleurs modèles
...
<center>
![](performance_glm.png){width=600px}
...
<center>
![](performance_gbm.png){width=600px}
