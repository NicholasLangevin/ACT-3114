...
## Variable endogène en fonction des variables explicatives

Il est intéressant de voir comment se comporte la variable endogène en fonction des différentes variables explicatives. Bien sûr, nos variables explicatives comme décrit plus tôt ne sont pas toutes de même type il en résulte donc qu'elles n'auront
...
Puisque la variable réponse **lapse** est une variable catégorielle pour laquelle deux valeurs sont possibles soit le renouvellement ou soit la résignation. Pour la suite on pourrait s'intéresser à modéliser la probabilité de renouveller ou bien de résigner. Pour résoudre ce problème il serait possible d'utiliser un modèle linéaire avec régression logistique. Dans ce cas, la prédiction de la régression corresponderait à la probabilité désiré. Si on s'intéresse plutôt à une prédiction tant qu'au renouvellement ou à la résignation. Dans ce cas là il serait possible d'utiliser encore le modèle linéaire avec régression logistique ou bien un autre modèle de classification supervisé. 
